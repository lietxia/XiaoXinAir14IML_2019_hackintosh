...
function SubmitData (FunNum, IsReboot) {
  ...
  // if (Data == "") {
  //   PopupInfo ("Warning", "Nothing changed!");
  ...
  g_WsSocket.onmessage = function(evt) {
    Result = evt.data;
    switch (parseInt(FunNum)) {
      case 1: // Save changes and exit
        if (Result >= 0) {
          alert('Please close browser manually.');
        } else {
      ...
      case 5: // Load optimal defaults
        if (Result != 0) {
          ...
          alert ("Load complete! Please reboot to make changes work.");
        }
        break;
  ...
  try {
    g_WsSocket.send(parseInt(FunNum));  //First, send the method
    g_WsSocket.send(Data);              //Second, send the configuration
    g_WsSocket.send('EndOfStream');     //Finally, tell server the operation is done.
    ...
    SetPopupContent ("Something wrong in submiting data!<br/>Please upgrade borwser or contact with administrator.");
  }
  SetCtrlButtons (POPUP_WIN_BTN_OK, ClosePopup);
}
...
//Alvin@Insyde: 20131128: BEGIN
function ConnWsServer() {
  g_WsSocket = new WebSocket('ws://127.0.0.1:3000');
  //g_WsSocket.onopen    = function(evt) { onOpen(evt) };
  //g_WsSocket.onclose   = function(evt) { onClose(evt) };
  g_WsSocket.onerror   = function(evt) { onError(evt) };
  //g_WsSocket.onmessage = function(evt) { onMessage(evt) };
}
...
//window.addEventListener("load", ConnWsServer, false);
//Alvin@Insyde: 20131128: END
